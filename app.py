        ...
        if user['status'] == 'main':
            ...
            if option == 1:
             ...
             for data in profile.find({"$and":[{'Subject':Subject,'Grade':Grade,'item':item,'pin_code':code}]}):
                 ...
                 res.message(f"Here we go *{selected_profile['name']}* is great *{Subject}* teacher for *{Grade}* in your area \n\n. *{selected_profile['prof']}*\n\n. To connect with the tutor you can call:{selected_profile['number']}\n\n To connect on Whatsapp click on the link\n\n:{selected_profile['chat']}")
             users.update_one({'number':number},{"$set":{"status":"main"}})
